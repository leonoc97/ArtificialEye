...
bag_on_chair_overlap_threshold = 0.5  # 60% overlap threshold for bag on chair
...
def start_webcam(last_configurations):
    ...
    print("LAST CONFIGURATION", last_configurations)
    print("THIS IS THE STARTUP SCRIPT")
    ...
    while True:
        ...
        # Sort objects vertically to determine front/back
        merged_objects = sorted(merged_objects, key=lambda obj: (obj['bbox'][1] + obj['bbox'][3]) / 2)

        # Dynamically determine the number of objects for front and back based on total detected objects
        total_objects = len(merged_objects)
        expected_objects_per_group = max(1,
                                         total_objects // 2)  # Ensure at least one object per group if any are detected

        # Divide into front and back groups based on vertical sorting
        half_index = total_objects // 2
        back_objects = merged_objects[:half_index]
        front_objects = merged_objects[half_index:]

        # Sort each group horizontally
        back_objects_sorted = sorted(back_objects, key=lambda obj: (obj['bbox'][0] + obj['bbox'][2]) / 2)
        front_objects_sorted = sorted(front_objects, key=lambda obj: (obj['bbox'][0] + obj['bbox'][2]) / 2)

        # Adjust the number of horizontal labels based on the actual number of objects in each group
        horizontal_labels = ['left', 'center', 'right']
        num_back_labels = min(len(back_objects_sorted), len(horizontal_labels))
        num_front_labels = min(len(front_objects_sorted), len(horizontal_labels))

        # Assign labels to back objects
        for i, obj in enumerate(back_objects_sorted):
            obj['vertical_position'] = 'back'
            if i < num_back_labels:
                obj['horizontal_position'] = horizontal_labels[i]
            else:  # If more objects than labels, default to 'center' or handle as needed
                obj['horizontal_position'] = 'center'  # Or another default/handling strategy

        # Assign labels to front objects
        for i, obj in enumerate(front_objects_sorted):
            obj['vertical_position'] = 'front'
            if i < num_front_labels:
                obj['horizontal_position'] = horizontal_labels[i]
            else:  # If more objects than labels, default to 'center' or handle as needed
                obj['horizontal_position'] = 'center'  # Or another default/handling strategy
